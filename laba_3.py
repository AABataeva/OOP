...
class Array3d:
    ...
    def GetValues1(self, i):  # Получаем срез по одному приближению = двумерный массив
        result = ""
        for j in range(self.dim1):
            result += "\n"
            for k in range(self.dim2):
    ...
    def SetValues1(self, i, array):  # В 1 приближении на 0 элемент ставим необходимый двумерный массив
        for k in range(self.dim2):
            for j in range(self.dim1):
                self.arr[self.transform_index(i, j, k)] = array[k][j]
        return self.arr

    def SetValues2(self, i, j, array):  # В 2 приближении на элемент [0][0] ставим необходимый одномерный массив
        for k in range(self.dim2):
